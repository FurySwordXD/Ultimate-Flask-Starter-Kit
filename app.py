...
def login():
    ...
    if request.method == "GET":
        if not session.get("user"):
            return render_template("generic_form.html", fields=User.create_fields(), action_url="/login/")
        else:
            return redirect("/")
        ...
        if user:
            ...
            if user.password == sha256(str(request.form["password"]).encode()).hexdigest():
                ...
                return render_template("generic_form.html", fields=User.create_fields(), action_url="/login/", errors=["Username and password do not match!"])
        return render_template("generic_form.html", fields=User.create_fields(), action_url="/login/", errors=["User does not exist!"])
